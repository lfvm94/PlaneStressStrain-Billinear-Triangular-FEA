...
[Edof,Coord,Ex,Ey,LeftSide_nodes,TopSide_nodes,RightSide_nodes,...
    BottomSide_nodes]=TunnelMeshGen(H,B,D,b,h,rs,18,12,1);
...
ngp=1; % Number of Gauss points for numerical integration for each
       % Triangular Finite Element. Only 1 is allowed for this 2D linear
       % triangle element
...
for i=1:nElements
    ...
    [ Ke, fe ] = plan3triangKelFbel(Ex(i,:),Ey(i,:),ep,eq);
     for j=2:7
...
StressStrainCompatibilityTriang3(nElements,Ed,ngp,E,v,typeAnalysis,...
Ex,Ey,Edof,Coord);
...
xlabel('Width [cm]')
ylabel('Height [cm]')
title('Displacement xx Distribution (cm)');

figure(3);
fill(Ex',Ey',Edu(:,[2 4 6])');
axis equal;  
axis image;
shading interp;
colormap 'jet';
...
xlabel('Width [cm]')
ylabel('Height [cm]')
title('Displacement yy Distribution (cm)');
